...
public class ProjectIssuesController {
    ...
    @Operation(summary = "프로젝트에 할당된 에픽 간단한 조회", description = "프로젝트에 할당된 에픽들을 간단하게 조회하는 API",
        responses = {
            @ApiResponse(responseCode = "200", description = "에픽 조회 성공", content = @Content(schema = @Schema(implementation = IssueResponseDto.ReadSimpleIssue.class)))}
    )
    ...
    @Operation(summary = "프로젝트에 할당된 스토리 간단한 조회", description = "프로젝트에 할당된 스토리들을 간단하게 조회하는 API",
        responses = {
            @ApiResponse(responseCode = "200", description = "스토리 조회 성공", content = @Content(schema = @Schema(implementation = IssueResponseDto.ReadSimpleIssue.class)))}
    )
    ...
    @Operation(summary = "프로젝트에 할당된 테스크 간단한 조회", description = "프로젝트에 할당된 테스크들을 간단하게 조회하는 API",
        responses = {
            @ApiResponse(responseCode = "200", description = "테스크 조회 성공", content = @Content(schema = @Schema(implementation = IssueResponseDto.ReadSimpleIssue.class)))}
    )
